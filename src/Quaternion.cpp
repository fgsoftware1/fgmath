...
namespace fgengine
{
	namespace math{
		...
		math::Quaternion::Quaternion(float uS, Vector3 & uV){ }

		math::Quaternion::~Quaternion(){ }

		math::Quaternion::Quaternion(const Quaternion & value){
			s=value.s;
			v=value.v;
		};


		math::Quaternion& math::Quaternion::operator=(const Quaternion & value){
			s=value.s;
			v=value.v;
		...
		void math::Quaternion::operator-=(const Quaternion & q){
			s-=q.s;
			v-=q.v;
		}
		...
		math::Quaternion math::Quaternion::multiply(const Quaternion & q) const{
			float scalar=s * q.s - v.dot(q.v);
		...
		void math::Quaternion::operator*=(const float value){
			s*=value;
			v*=value;
		}

		math::Quaternion math::Quaternion::operator*(const float value) const{
		...
		math::Quaternion math::Quaternion::operator*(const Vector3 & uValue) const{
		...
		float math::Quaternion::dot(Quaternion & q){
			return s * q.s + v.x * q.v.x + v.y * q.v.y + v.z * q.v.z;
		}

		float math::Quaternion::norm(){
			float scalar=s * s;
			float imaginary=v * v;
		...
		math::Quaternion math::Quaternion::inverse(){
		...
		void math::Quaternion::inverse(Quaternion & q){
			Quaternion dummy=q.inverse();

			q=dummy;
		}

		void math::Quaternion::convertToUnitNormQuaternion(){
		...
		math::Matrix3 math::Quaternion::transformQuaternionToMatrix3(){
			Matrix3 m;
		...
		void math::Quaternion::transformEulerAnglesToQuaternion(float x, float y, float z){
			x=DegreesToRad(x);
			y=DegreesToRad(y);
			z=DegreesToRad(z);
		...
		math::Vector3 math::Quaternion::transformQuaternionToEulerAngles(){
			float x=0.0;
			float y=0.0;
			float z=0.0;
		...
		void math::Quaternion::transformMatrix3ToQuaternion(Matrix3 & uMatrix){
